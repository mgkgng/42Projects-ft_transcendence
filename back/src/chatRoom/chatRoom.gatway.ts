...
export class ChatRoomService {
	...
	async creat_room(@MessageBody() data: any, @Request() req, @ConnectedSocket() client : Socket)
	{
		...
		try{
			...
			client.emit("new_room", {	id_public_room: new_chat_room.id_public_room, room_name: name, is_password_protected: is_password_protected, is_admin: true, is_private: data.is_private	});
		} catch (e) {
	...
	async append_user_to_room(@MessageBody() data: any, @ConnectedSocket() client: Socket, @Request() req)
	{
		try{
			...
			client.emit("set_room_visible", {id_public_room: room.id_public_room});
			client.emit("append_user_to_room", {id_public_room: room.id_public_room, is_admin: false, username: user.username});
		}
		catch(e){
	...
	async get_users_room(@MessageBody() data: any, @ConnectedSocket() client: Socket, @Request() req)
	{
		try{
			...
			try{
				...
				client.emit('get_users_room', {users: end, id_public_room: data.id_public_room});
			} catch (e) {
	...
	async getMessageRoom(@MessageBody() data: any, @ConnectedSocket() client: Socket, @Request() req) 
	{
		try{
			...
			try{
				...
				client.emit('get_message_room', {messages : res, id_public_room: data.id_public_room} )
			} catch (e) {
