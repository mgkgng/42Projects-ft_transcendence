...
export class AuthService {
	...
	async validateUser42(code: string): Promise<any> 
	{
		...
		//const rep = await this.httpService.post("https://api.intra.42.fr/oauth/token", {data: {grant_type: "authorization_code", client_id: iud, client_secret: secret, code: code, redirect_uri: "http://localhost:3001"}}).toPromise();
		try {
			const rep = await this.httpService.post("https://api.intra.42.fr/oauth/token", "grant_type=authorization_code&code=" + code + "&client_id=" + iud + "&client_secret=" + secret + "&redirect_uri=http://localhost:3001").toPromise();
